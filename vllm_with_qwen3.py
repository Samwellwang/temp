    ...
    if content:
        ...
        if result:
            try:
                ...
                #判断 返回格式是否是list
                for key in ['姓名', '联系方式', '地址', '工作单位', '职位', '邮箱', '所属部门', '职称', '传真',
                            '个人主页', '社交信息']:
                    key_value_list = data.get(key, None)
                    if not key_value_list:
                        continue
                    if type(key_value_list) is not list:
                        del data[key]
                        continue
                    tmp_key_value_list = []
                    for key_value in key_value_list:
                        if type(key_value) is str:
                            tmp_key_value_list.append(key_value)
                        if type(key_value) is dict:
                            tmp_key_value_list.append(json.dumps(key_value))
                    data[key] = tmp_key_value_list
...
if __name__ == '__main__':
    os.environ["llm_host"] = "http://xxxx:8011/v1/"
    result = content_ner_with_llm(
